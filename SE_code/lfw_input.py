...
def filter_dataset(dataset, min_images_per_label=10):
    ...
    for i in range(len(dataset)):
        if len(dataset[i].image_paths) < min_images_per_label:
            ...
            filtered_dataset_length += len(dataset[i].image_paths)
    logger.info('Skipping ' + str(skipped) + ' classes with insufficient amounts of images')
