...
def main(input_directory, model_path, id_classifier_output_path, gen_classifier_output_path, age_classifier_output_path, batch_size, num_threads,
         num_epochs, min_images_per_labels, split_ratio, embed, train_path, is_train=True, is_yes=True):
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        if is_train:
            ...
            images, labels, class_names, image_paths = _load_images_and_labels(test_set, image_size=160, batch_size=batch_size,
        ...
        emb_array, label_array = _create_embeddings(embedding_layer, images, labels, images_placeholder,
                                                    phase_train_placeholder, sess, is_train, len(image_paths),
                                                    batch_size, num_epochs, is_yes, coord, threads, embed)
        ...
        if is_train:
            ...
            _evaluate_classifier(emb_array, label_array, classifier_filenames, image_paths)
...
def _create_embeddings(embedding_layer, images, labels, images_placeholder, phase_train_placeholder, sess,
                       train, img_paths, batch_size, num_epochs, override, coord, threads, embed):
    """
    ...
    ok, it_total = Estimate.estimate(img_paths, batch_size, num_epochs, override, train)
    ...
    if not ok:
        ...
        sess.close()
        sys.exit()
    if os.path.exists("/src/embeddings.pkl") and not embed:
        ...
        try:
            ...
            while True:
                ...
                logger.info(str(math.floor((i/it_total)*100)) + '% - Processed iteration {} batch of size: {}'.format(i, len(batch_labels)))
                i += 1
...
def _evaluate_classifier(emb_array, label_array, classifier_filename, image_paths):
    ...
    try:
        with open(classifier_filename[0], 'rb') as IDf:
            ...
            for i in range(len(best_class_indices)):
                results.append([i, image_paths[i], class_names[best_class_indices[i]], best_class_probabilities[i]])
        ...
        with open(classifier_filename[1], 'rb') as GENf:
            ...
            for i in range(len(best_class_indices)):
                try:
                    ...
                    results.append([i, None, None, None, class_names[best_class_indices[i]], best_class_probabilities[i]])
        ...
        with open(classifier_filename[2], 'rb') as AGEf:
            ...
            for i in range(len(best_class_indices)):
                try:
                    ...
                    results.append([i, None, None, None, None, None, class_names[best_class_indices[i]], best_class_probabilities[i]])
    ...
    for z in results:
        ...
        results_dict = {
            ...
            "AGE": z[6],
            "AGE-prob": z[6],
            "AGE-case": decide_case(z[7])
        }
        print(results_dict)
        # TODO 12/4 testing!,train gender a little more, add toss-up if possibilty is under 60, guess <75
        # TODO future: make single image feed line with returns, remove nice wording from result log
...
if __name__ == '__main__':
    ...
    main(input_directory=args.input_dir, model_path=args.model_path, id_classifier_output_path=args.id_classifier_path,
         gen_classifier_output_path=args.gen_classifier_path, age_classifier_output_path=args.age_classifier_path,
         ...
         is_yes=args.is_yes, embed=args.embed, train_path=args.train_classifier_path)
